...
impl Buffer {
    ...
    pub fn combine(&mut self, (col, row): VisualPosition, ch: char) -> bool {
        self[row].cells[col].combine(ch)
    }
    ...
    pub fn delete(&mut self, (col, row): VisualPosition, mut n: usize, pen: &Pen) {
        ...
        // xterm DCH preserves the line's wrapped relationship. In particular,
        // checkpoint repaint under insert mode must not split a wrapped line.
    }
