...
pub struct Terminal {
    ...
    xtwinops: bool,
    last_print_position: Option<(usize, usize)>,
    pub recovery_overflow: bool,
}
...
impl Terminal {
    ...
    fn print(&mut self, mut ch: char) {
        ...
        if crate::width::char_width(ch) == 0 {
            if let Some(position) = self
                .last_print_position
                .filter(|position| crate::width::char_width(self.buffer[*position].char()) > 0)
            {
                if !self.buffer.combine(position, ch) {
                    self.recovery_overflow = true;
                }
                self.dirty_lines.add(position.1);
                return;
            }
        }
    ...
    fn try_print(&mut self, ch: char) -> PrintResult {
        ...
        if self.insert_mode {
            self.buffer.shift_right(
                (self.cursor.col, self.cursor.row),
                crate::width::char_width(ch).max(1).into(),
                self.pen,
            );
        }
        ...
        {
            Some(width) => {
                self.last_print_position = Some((self.cursor.col, self.cursor.row));
                PrintResult::Printed(width)
            }
    ...
    fn wrap_and_print_at_line_start(&mut self, ch: char) {
        ...
        if let Some(width) = self.buffer.print((0, self.cursor.row), ch, self.pen) {
            self.last_print_position = Some((0, self.cursor.row));
            self.cursor.col = width;
        } else {
            self.cursor.col = 0;
        }
    ...
    fn print_at_line_end(&mut self, ch: char) {
        ...
        if self.cursor.col > 0 {
            ...
            if printed {
                self.last_print_position = Some((col, row));
            }

            if !printed && col > 0 {
                self.buffer.print((col - 1, row), ch, self.pen);
                self.last_print_position = Some((col - 1, row));
            }
        }
    ...
    fn rep(&mut self, n: u16) {
        if self.cursor.col > 0 {
            ...
            let chars: Vec<char> = self.buffer[(col, row)].chars().collect();
    ...
    pub fn dump(&self) -> Vec<Function> {
        ...
        if self.cursor.col >= self.cols {
            ...
            if occupancy == Occupancy::Single {
                ...
                funs.extend(last_cell.chars().map(Function::Print));
                ...
                funs.extend(prev_cell.chars().map(Function::Print));
            }
        }
        ...
        // xterm's Unicode join state (and REP source) is established by a print,
        // not a cursor/SGR sequence. Repaint the last cell LAST so a combining
        // character in the next raw chunk joins exactly as it did before attach.
        if let Some((col, row)) = self.last_print_position {
            let relative_row = if self.origin_mode {
                row.saturating_sub(self.top_margin)
            } else {
                row
            };
            funs.push(Function::Cup(as_u16(relative_row + 1), as_u16(col + 1)));
            let cell = &self.buffer[(col, row)];
            if self.insert_mode {
                funs.push(Function::Dch(cell.width().into()));
            }
            funs.extend(cell.chars().map(Function::Print));
        }
...
fn dump_cells(cells: &[Cell], funs: &mut Vec<Function>) {
    ...
    while i < cells.len() {
        let cell = &cells[i];
        let mut run_len = 1;
        if cell.chars().count() == 1 && crate::width::char_width(cell.char()) > 0 {
            while i + run_len < cells.len() && cells[i + run_len] == *cell {
                run_len += 1;
            }
        }
        if run_len > 5 {
            funs.push(Function::Print(cell.char()));
            funs.push(Function::Rep(as_u16(run_len - 1)));
        } else {
            for cell in &cells[i..i + run_len] {
                if crate::width::char_width(cell.char()) == 0 {
                    // A standalone zero-width codepoint must not join the
                    // preceding cell merely because dump coalesced print runs.
                    funs.push(to_sgr(cell.pen()));
                }
                funs.extend(cell.chars().map(Function::Print));
            }
        }
...
mod tests {
    ...
    fn execute_dch() {
        ...
        // xterm DCH preserves the existing soft-wrap relationship.
        assert_eq!(wrapped(&term), vec![true, false]);
