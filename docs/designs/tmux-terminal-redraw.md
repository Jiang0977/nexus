...
5. **输入路由**：真实 tmux 重绘恢复其协商的鼠标模式，优先由 xterm 编码标准鼠标输入，不凭标题、2026 或 alternate screen 推断应用身份。Grok legacy 分支的移除及非标准应用 profile/manual fallback 需要独立覆盖，不因重绘完成就自动宣称完成。
