...
当前源码树的默认启动链已经切到 [`start.sh`](/home/demo/workspace/typescript/nexus4cc/start.sh) -> Rust `nexus-server`；旧 Node 后端入口、旧编译产物入口，以及对应的后端业务服务源码都已从当前分支删除。保留这份文档，只是为了说明当时为什么先做 Node 边界清理，再继续推进整体 Rust 化。
