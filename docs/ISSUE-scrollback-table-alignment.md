...
在终端（tmux）里，等宽字体下中文占 2 列宽、英文占 1 列宽，终端模拟器能正确按列对齐。但在“普通纯文本”环境中（比如记事本、邮件、网页 `<pre>` 标签），即使用了等宽字体，不同系统和渲染引擎对中英混排的宽度计算可能不一致，表格就会错位。
...
**如果目标是分享或文档**，不要用纯文本表格，改用 Markdown 表格（渲染器会处理对齐）、HTML `<table>`，或者导出为 CSV / Excel。


**如果是自己写的 CLI 工具**，用 `wcwidth` / `string-width` 这类库来计算实际显示宽度再做 padding，而不是简单用 `String.length`。

你截图里这个看起来是某个旧项目的 Claude Code 审计输出？如果你需要把这些表格内容导出成可分享的格式，我可以帮你转。

---

对的。我的是旧项目。claude code 运行在 tmux 中通过浏览器显示，和终端交互。但是遇到这个问题，你有什么解决办法吗？
