...
- 通用 TUI 终端滚动能力与模式快照（原 P1，L；本机约定范围验收完成）
  ...
  - 当前进度：native 完整 checkpoint、共享最小尺寸、Unicode 11 一致性、显式 channel scroll profile 已实现；Rust 222 项、Node 215 项及完整门禁通过。2026-09-10 09:36 CST 已部署并重启 nexus 与 native supervisor，指定 HTTPS 入口 11 项验收通过，含真实登录上传、分屏、移动视口、刷新/重连、60 次握手、Codex/Grok 启动画面恢复；测试项目和上传已清理。见 [最终验收](docs/verification/native-production-acceptance-2026-09-10.md) 与 [恢复边界](docs/designs/native-terminal-checkpoint.md)。
