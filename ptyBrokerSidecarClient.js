...
const DEFAULT_SIDECAR_PATH = join(dirname(fileURLToPath(import.meta.url)), 'ptyBrokerSidecarProcess.js')
...
/** @param {PtyBrokerSidecarClientOptions} options */
export function createPtyBrokerSidecarClient(options = {}) {
  const {
  ...
  function buildExitMessage(code, signal) {
    return `broker sidecar exited (code=${code ?? 'null'}, signal=${signal ?? 'null'})`
  }
  ...
  child.on('message', (message) => {
    ...
    if (message.kind === 'response') {
      ...
      if (message.ok) {
        ...
        pending.reject(new Error(message.error?.message || 'broker sidecar request failed'))
      }
      return
    }
  ...
  child.on('error', (error) => {
    if (closed) return
    log.error?.('broker sidecar process error:', error)
  ...
  function request(method, params) {
    if (closed) {
      return Promise.reject(new Error('broker sidecar is not available'))
    }

    const id = `broker_req_${++requestCounter}`
  ...
  return {
    ...
    handleConnectionMessage(params) {
      notify('handleConnectionMessage', params)
    },
